...
utils::write.csv(results, file.path(output_dir, "mediation_comparator_validation.csv"), row.names = FALSE)

# Per engine x scenario x loading summary.
...
summary <- do.call(rbind, lapply(groups, function(group) data.frame(
  ...
  n_rows = nrow(group),
  available_rate = mean(!is.na(group$indirect)),
  mean_true_indirect = mean(group$true_indirect),
  mean_indirect = mean(group$indirect, na.rm = TRUE),
...
utils::write.csv(summary, file.path(output_dir, "mediation_comparator_summary.csv"), row.names = FALSE)

# Headline matrix: one row per engine, pooled across scenarios.
...
utils::write.csv(matrix, file.path(output_dir, "mediation_comparator_matrix.csv"), row.names = FALSE)

metadata <- data.frame(
  run = "mediation_comparator_validation",
  workers_requested = workers,
  jobs = nrow(manifest) * 10L,
  rows_written = nrow(results),
  elapsed_seconds = round(elapsed, 2),
  stringsAsFactors = FALSE
)
utils::write.csv(metadata, file.path(output_dir, "mediation_comparator_run_metadata.csv"), row.names = FALSE)
