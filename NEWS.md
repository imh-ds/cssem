...
* Added a G4 reliability/shape-selection bootstrap validation study and a
  ...
  replication grid and runtime provenance before summarizing. The completed
  confirmation (Actions run `35954818332`) passed its registered bias,
  coverage, failure, and bootstrap-availability gates for the four declared
  scenarios; results and limits are recorded in `docs/validation-g15.md`. The
  sample-size planner remains gated because G6 outer-validation metrics lack an
  independently calibrated uncertainty method. Commits: `486c949`, `9b9345a`,
  `cb20ccf`, `3f340fb`, `f9f984c`, `0620bbc`, `50b6b7f`, `f3d13a0`, `72cf6ac`,
  `4dbd2f9`.
...
* Added `bootstrap_model(resample = "cluster")` for complete-unit resampling,
  ...
  the default.
* Hardened cluster bootstrap refits so duplicated source units remain in one
  measurement fold, explicit cluster labels reach point callbacks, and
  replicate metadata separates distinct source units from draw occurrences.
...
* Added `group_comparison()` for reference-directed structural path contrasts
  ...
  `183cc0b`, `845e274`, `23551e2`, `27e84c1`.
...
* Added `prediction_assessment()` with RMSE, MAE, R-squared, calibration
  ...
  `f7a4cd1`, `566543f`, `b58d4df`, `cf60de7`.
