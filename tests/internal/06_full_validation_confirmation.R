...
measurement_results <- validate_measurement(
  measurement_manifest("screening"), reps = 5, seed = 2026, workers = workers
)
...
structural_results <- validate_structure(
  structural_manifest("screening"), reps = 5, seed = 3026, workers = workers
)
structural_elapsed <- proc.time()[["elapsed"]] - structural_started
report <- validation_report(measurement_results, structural_results)
