...
test_that("conditional_slopes reports conditional slopes and a Johnson-Neyman region", {
  ...
  expect_equal(length(ss$johnson_neyman$ci_low), length(ss$johnson_neyman$grid))
  expect_equal(length(ss$johnson_neyman$ci_high), length(ss$johnson_neyman$grid))
  expect_true(all(is.finite(ss$johnson_neyman$slope)))
  expect_identical(ss$johnson_neyman$interval_basis, "percentile_bootstrap")
  slope_plot_data <- plot_data(ss, type = "slopes")
  jn_plot_data <- plot_data(ss, type = "johnson_neyman")
  expect_true(all(slope_plot_data$interval_basis == "percentile_bootstrap"))
  expect_true(all(is.finite(jn_plot_data$moderator_value)))
  expect_true(all(is.finite(jn_plot_data$ci_low) & is.finite(jn_plot_data$ci_high)))
  path_plot_data <- plot_data(association, type = "paths")
  expect_true(any(path_plot_data$edge_type == "interaction_input"))
  curve_plot_data <- plot_data(association, type = "curve", outcome = "Y", predictor = "M")
  expect_equal(range(curve_plot_data$x), range(fx$scores$M))
